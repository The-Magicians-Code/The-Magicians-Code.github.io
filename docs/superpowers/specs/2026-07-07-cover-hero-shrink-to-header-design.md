...
- **No fade-to-white (revised).** The resting `.cover` stays **opaque** through the morph and grows with the box, so the cover image transforms *continuously* into the full-screen hero rather than cross-fading through paper. The cloned `.cs-hero` image is given the resting cover's **exact oversized crop** (`top:-56px; height:calc(100%+112px)`, matching `.has-cover .cover-img`), so when it reveals over the grown cover at settle it is pixel-identical — no seam. On close the hero fades out to reveal the same still-opaque cover, which shrinks back with the box (white-free close too). Only `.cover-resttitle` still blurs out (a title cross-dissolve into the hero title, not a cover fade). *(This replaced the original fade-the-cover-to-paper handoff after the full-viewport hero landed — the fade was a vestige of the pre-hero design.)*
