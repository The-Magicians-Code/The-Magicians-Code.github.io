...
- **No fade-to-white (revised).** The resting `.cover` stays **opaque** through the morph and grows with the box, so the cover image transforms *continuously* into the full-screen hero rather than cross-fading through paper. The cloned `.cs-hero` is `height:100%` + `opacity:1` (it tracks the box — grows on open, shrinks on close — and simply appears over the still-visible resting cover, no reveal fade) and its image is given the resting cover's **exact oversized crop** (`top:-56px; height:calc(100%+112px)`, matching `.has-cover .cover-img`), so the swap is pixel-identical — no seam. Only `.cover-resttitle` blurs out (a title cross-dissolve into the hero title, not a cover fade). *(This replaced the original fade-the-cover-to-paper handoff after the full-viewport hero landed.)*
- **Content reads on paper, not the cover.** Because the resting cover now stays opaque behind the (otherwise transparent) `.card-body`, the case study would sit on the cover image. So `.card-body` carries a **`var(--paper)` background gated by lifecycle**: transparent during the morph and the collapse (`is-collapsing`) — when the resting cover must show the growing/shrinking cover — and opaque when settled (`is-content-in`) and through the close content-fade (`is-closing-content:not(.is-collapsing)`). On close only the pinned bar fades; the hero shrinks with the box.
